...
from neuralconstitutiveeq.tipgeometry import TipGeometry, Conical, Spherical
from neuralconstitutiveeq.utils import beta, to_parameter

# %%
# tip = Spherical(R=5e-8, h = 6.2e-6)
tip = Conical(np.pi / 10)
print(tip.alpha_corrections(6.2e-6, 4))
...
class PLRTriangularIndentation(nn.Module):
    ...
    def t1(self, t):
        t1 = t - (t - self.t_max) * torch.tensor(2.0) ** (1 - self.gamma).reciprocal()
...
plt.plot(t.numpy(), F_PLR.clone().detach().numpy(), ".")

# %%
F._approach(torch.tensor([0.0]).view(-1))
# %%
F(torch.tensor([0.0]).view(-1))
# %%
F_PLR
# %%
t
